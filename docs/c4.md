...
This document describes the system using the **C4 model**, focusing on the **Context** and **Container** levels.
...
- Requests daily meal suggestions (Cooking Plan)
- Accepts suggestions (consumes inventory)
- Generates shopping lists
- Accesses the system via the Web Demo (current) and a future mobile client
...
**PostgreSQL Database**
- Stores users, households, inventory, recipes and suggestions
...
|  - Auth (JWT) + Household authorization     |
...
- Enforce authentication and household authorization for protected routes
...
|  - Routes / Controllers                     |
|  - Zod validation                           |
|  - Auth (JWT) + Household access checks     |
...
- A secured backend foundation suitable for future frontend integration
